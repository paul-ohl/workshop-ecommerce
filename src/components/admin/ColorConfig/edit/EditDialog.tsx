...
const EditDialog: React.FC<EditDialogProps> = ({ open, onClose, configId }) => {
  ...
  const [colorConfig, setColorConfig] = useState<any>(null);
  ...
  useEffect(() => {
    if (data && configId) {
      const foundConfig = data?.[0]?.colorsConfigs?.find(
        (conf: any) => conf._id === configId
      );

      if (foundConfig) {
        setColorConfig(foundConfig);
        setFormData({
  ...
  const resetForm = () => {
    if (colorConfig) {
      setFormData({
        ...
        isMultiSelection: colorConfig.isMultiSelection || false,
      });
      setAccordionItems(colorConfig.refs || []);
      setIsModified(false);
    }
  };
